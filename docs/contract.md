...
// 200
{
  ...
  "promotions": [
    { "productId": 7752,
      ...
      "promoMonths": 6,
      "regularPrice": 13200,                        // 특가 **종료 후** 월 요금
...
- **`regularPrice` 는 "N개월 이후 B원/월" 의 B 다.** 페이지 머리말의 앞 숫자(`월 19,800 원`)는
  **싣지 않는다** — 정가인지 특가인지 페이지가 말하지 않는다. 상품명이 "12개월간 990원 특가"인데
  앞 숫자가 31,900원인 경우가 있다(특가는 비교 카드에만 990원으로 나온다). **모르는 값은 안 옮긴다.**
...
- **실패는 행 단위다.** 한 상품을 못 읽어도 나머지는 돌아온다. `failures` 에 번호와 사유가 실린다.
  **BE 는 `failures` 에 오른 번호의 기존 행을 건드리지 않는다** — 못 읽은 것과 특가가 끝난 것은 다르다.
  행이 사라진 것으로 보고 지우면 멀쩡한 특가가 조용히 없어진다.
