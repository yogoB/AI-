...
- **`currentMonthlyTotal`(선택, 원)** — 지금 쓰는 요금제로 같은 구독을 유지했을 때의 실질월비용
  ...
  - **이 뺄셈이 이 서버가 하는 유일한 산술이다.** 절대 원칙 1은 금액 계산을 금지하지만, 이 차는
    BE 가 준 두 수의 관계를 말하기 위한 것이고 계약이 명시적으로 허용한다. 두 수가 같은 기준으로
    계산됐다는 보증은 BE 가 한다. **새 금액이 필요해지면 여기서 만들지 말고 BE 에 필드를 요청한다.**
